...
import { buyModalHide, sweepModalhide } from '../../redux/counterSlice';
...
const SweepModal = (props) => {
	...
	const sweepModal = useSelector((state) => state.counter.sweepModal);
	...
	const getAvgPrice = () =>{
        ...
        res=0;
        if(myopencarts.length > 0)
        res=(total/myopencarts.length).toFixed(2)
        return res;
    }
	...
	const getRange = () =>{
        ...
        if(myopencarts.length === 0)
            res=[0,0];
        if(myopencarts.length > 0)
            res=[min.toFixed(2),max.toFixed(2)];
        return res;
    }
	...
	return (
		<div>
			{/* <!-- Buy Now Modal --> */}
			<div className={sweepModal ? 'absolute fade show block -top-0 right-[1rem]' : 'modal fade'}>
				<div className="modal-dialog max-w-2xl w-96 ">
					<div className="modal-content dark:border-jacarta-600 border">
						<div className="modal-header">
							...
							<button type="button" className="btn-close" onClick={() => dispatch(sweepModalhide())}>
								<svg
						...
						<div className="modal-body p-4 pb-0">
							<div className='flex justify-between'>
								<div className="flex w-1/3 pr-4">
									<input type="number" className='dark:bg-jacarta-700 border-jacarta-100 hover:ring-accent/10 focus:ring-accent dark:border-jacarta-600 dark:placeholder:text-jacarta-300 w-full rounded-lg px-1 hover:ring-2 dark:text-white' min="0" max={cartLimit} value={myopencartscnt} onChange={(e)=>dispatch(cartsActions.updateCount(e.target.value))} readOnly/>
									<p className='pt-2 px-1 dark:text-jacarta-300'>Item</p>
								</div>
								<input type="range"  min="0" max={cartLimit} value={myopencartscnt} onChange={(e)=>dispatch(cartsActions.updateCount(e.target.value))} className='text-jacarta-700 hover:texttext-jacarta-700'/>
								<p className='px-2 pt-2 item-center'>Max {cartLimit}</p>
							</div>
							{/* <!-- Range --> */}
							<div className="dark:border-jacarta-600 border-jacarta-100 flex items-center justify-between border-b py-2.5">
								<div className='w-1/2 px-3'>
									...
									<div className="ml-auto flex">
										<span className="flex items-center whitespace-nowrap">
											...
											<span className="text-green font-medium tracking-tight">{getAvgPrice()} Ⓝ</span>
										</span>
								...
								<div className='w-1/2 border-l px-3 dark:border-jacarta-600'>
									...
									<div className="ml-auto flex">
										<span className="flex items-center whitespace-nowrap">
											...
											<span className="text-green font-medium tracking-tight">{`${getRange()[0]} ~ ${getRange()[1]}`} Ⓝ</span>
										</span>
							...
							<div className='scrollbar-custom dark:bg-jacarta-700 rounded-3lg dark:border-jacarta-600 border-jacarta-100 w-full max-h-40 h-40 overflow-y-auto'>
								{
