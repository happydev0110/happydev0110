...
const Edit_user = () => {
	...
	const [photoimgFile, setphotoimgFile] = useState(null);

	const checkDataField = (datalist) => {
	...
	//update user collection field button function
	const updateProfile = async () => {
		let status = false;
		let tmp_bannerImg, tmp_photoImg;
		if (bannerimgFile) {
			console.log(bannerimgFile,'banner Image')
			const url = `https://api.pinata.cloud/pinning/pinFileToIPFS`;
			const formData = new FormData();
			formData.append("file", bannerimgFile);
			let response = await axios.post(url, formData, {
				headers: {
					'Content-Type': `multipart/form-data; boundary= ${formData._boundary}`,
					'pinata_api_key': 'fe3060199e1632e4d52a',
					'pinata_secret_api_key': '1f4a05527f4e641e6fe037affea5fb577196dd8c34f7db093c0d27102f3e6ae7',
				}
			});
			tmp_bannerImg = "https://nearnaut.mypinata.cloud/ipfs/" + response.data.IpfsHash;
		} else
			tmp_bannerImg = bannerImg;

		if (photoimgFile) {
			const url = `https://api.pinata.cloud/pinning/pinFileToIPFS`;
			const formData = new FormData();
			formData.append("file", photoimgFile);
			let response = await axios.post(url, formData, {
				headers: {
					'Content-Type': `multipart/form-data; boundary= ${formData._boundary}`,
					'pinata_api_key': 'fe3060199e1632e4d52a',
			...
			tmp_photoImg = "https://nearnaut.mypinata.cloud/ipfs/" + response.data.IpfsHash;
		} else
			tmp_photoImg = bannerImg;

		status = checkDataField([accountId, displayName, email, bio, twitter])
		console.log(near)
		if (status) {
			let updateData = { address: accountId, photo: tmp_photoImg, displayName, customeUrl, email, bio, twitter, discord, instagram, bannerImg: tmp_bannerImg }
			await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/update_user`, {
				params: {
					...updateData,
					"verified": false
				},
			}, { headers: { authorization: near._authToken } })
			// toast('Successfully Updated!', {
			// 	position: toast.POSITION.BOTTOM_RIGHT,
			// 	className: 'foo-bar'
			// })
			toast.success("Successfully Updated!",{position:toast.POSITION.BOTTOM_RIGHT,className:'foo-bar'})
		}
	}

	const uploadbanner = (ev) => {
	...
	useEffect(() => {
        async function init() {
            ...
            let initdata = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/get_user`, {
                params: {
                    address: accountId,
                },
            })
            if(initdata?.data.data != null){
                ...
                const maindata = initdata.data.data;
                setphoto(maindata.photo);
                setbannerImg(maindata.bannerImg);
                setbio(maindata.bio);
                setcustomeUrl(maindata.customeUrl);
                setdiscord(maindata.discord);
                setdisplayName(maindata.displayName);
                setemail(maindata.email);
                settwitter(maindata.twitter);
                setinstagram(maindata.instagram);
            }
        }
	...
	return (
		<div>
			<Meta title="Profile || Astromarket" />
			<div className="pt-[5.5rem] lg:pt-24">
				{/* <!-- Banner --> */}
				<div className="relative">
					<img
						src={bannerImg ? bannerImg : '/images/user/banner.jpg'}
						alt="banner"
							...
							<div className="mb-12 md:w-1/2 md:pr-8">
								<div className="mb-6">
									...
									<input
										...
										isInvalid={displayName === "" ? true : false}
									/>
								</div>
								<div className="mb-6">
									<label className="font-display text-jacarta-700 mb-1 block text-sm dark:text-white">
									...
									<input
										...
										required
										value={customeUrl}
										onChange={(ev) => setcustomeUrl(ev.target.value)}
									/>
								</div>
								<div className="mb-6">
									<label className="font-display text-jacarta-700 mb-1 block text-sm dark:text-white">
								...
								<div className="mb-6">
									...
									<textarea
										...
										className="dark:bg-jacarta-700 border-jacarta-100 hover:ring-accent/10 focus:ring-accent dark:border-jacarta-600 dark:placeholder:text-jacarta-300 w-full rounded-lg py-3 hover:ring-2 dark:text-white px-3"
										required
										placeholder="Please type your bio information."
										value={bio}
										onChange={(ev) => setbio(ev.target.value)}
										isInvalid={bio === "" ? true : false}
									></textarea>
								</div>
								<div className="mb-6">
									<label className="font-display text-jacarta-700 mb-1 block text-sm dark:text-white">
									...
									<div className="relative">
										...
										<input
											...
											value={discord}
											onChange={(ev) => setdiscord(ev.target.value)}
										/>
									</div>
									<div className="relative">
										<svg
								...
								<button className="bg-accent shadow-accent-volume hover:bg-accent-dark rounded-full py-3 px-8 text-center font-semibold text-white transition-all" onClick={updateProfile}>
									Update Profile
								</button>
							</div>
							{/* <!-- Avatar --> */}
							<div className="flex space-x-5 md:w-1/2 md:pl-8">
								<form className="shrink-0">
									<figure className="relative inline-block">
										<Image
											src={photo ? photo : '/images/user/user_avatar.gif'}
