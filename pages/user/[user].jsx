...
const User = () => {
	...
	const [upgraded, setupgraded] = useState(1);
	const [userData, setUserData] = useState({})
	const [collectionInfo, setCollectionInfo] = useState(null);

	const getCollectionInfo = async () => {
		try {
			const tmpinfo = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/collections`, {
				params: {
					owner_id: pid
				},
			})
			const initdata = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/get_user`, {
				params: {
					address: pid,
				},
			})
			...
			if (tmpinfo?.data?.data.results)
				setCollectionInfo(tmpinfo.data.data);
			setupgraded(Math.random() / 9 + 1)
		} catch (error) {
			console.log(error);
	...
	const defaultBannerUrl = '/images/option2_bg_profile.jpg'
	const defaultPhotoUrl = '/images/avatars/avt-2.jpg'
	return (
		<>
			<Meta title="User || Astromarket" />
			{/* <!-- Profile --> */}
			<div className="pt-[5.5rem] lg:pt-24">
				...
				<div className="relative h-[18.75rem]">
					{
						userData ?
							<Image src={userData.bannerImg ? userData?.bannerImg : defaultBannerUrl} alt="" layout="fill" objectFit="cover" /> : <Image src={defaultBannerUrl} alt="" layout="fill" objectFit="cover" />
					}
					{/* <img src={userData.bannerImg ? userData?.bannerImg : defaultBannerUrl} alt='' /> */}
				</div>

				<section className="dark:bg-jacarta-800 bg-light-base relative pb-12 pt-28">

					<div className="absolute left-1/2 top-0 z-10 flex -translate-x-1/2 -translate-y-1/2 items-center justify-center">
						<figure className="bg-white relative h-40 w-40 dark:border-jacarta-600 rounded-xl border-[5px] border-white">
							<Image
								src={userData?.photo ? userData?.photo : defaultPhotoUrl}
								// alt={title}
								layout="fill"
								objectFit="contain"
